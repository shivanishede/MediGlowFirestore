...
export const companiesService = {
    ...
    async add(name) {
        // Use name as doc ID (lowercased + trimmed) to prevent duplicates
        const docId = name.trim().toLowerCase().replace(/\s+/g, '_');
        const docRef = doc(db, "companies", docId);
        // setDoc with merge:true is idempotent — won't duplicate if already exists
        return await setDoc(docRef, { name: name.trim(), created_at: serverTimestamp() }, { merge: true });
    },
    async delete(name) {
        const docId = name.trim().toLowerCase().replace(/\s+/g, '_');
        return await deleteDoc(doc(db, "companies", docId));
    }
};